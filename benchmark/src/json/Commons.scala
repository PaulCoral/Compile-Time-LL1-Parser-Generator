...
object TokenClass {
  ...
  given ToExpr[TokenClass] with {
    def apply(tc: TokenClass)(using Quotes):Expr[TokenClass] = tc match {
      case SeparatorClass(v) => '{SeparatorClass(${Expr(v)})}
      case BooleanClass => '{BooleanClass}
      case NumberClass => '{NumberClass}
